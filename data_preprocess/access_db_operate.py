...
def get_variables_node(db):
    list_parameters_node = []
    query_iddecl_str = 'queryNodeIndex("type:IdentifierDeclStatement")'
    ...
    if results != []:
        for re in results:
            ...
            if (code.find('*') != -1) or (code.find(' [ ') != -1):
                list_parameters_node.append(re)
    ...
    return list_parameters_node


def get_control_node(db):
    list_control_node = []
    query_param_str = 'queryNodeIndex("type:IfStatement")'
    results = db.runGremlinQuery(query_param_str)
    if results != []:
        for re in results:
            code = re.properties['code']
            if code.find('if ') != -1:
                list_control_node.append(re)

    return list_control_node
...
def getALLNodes(db, func_id):
    query_str = 'queryNodeIndex("functionId:%s")' % func_id
    ALLNodes = db.runGremlinQuery(query_str)
...
def drawGraph(db, edges, func_entry_node, graph_type):
    ...
    node_type = []
    for edge in edges:
        start_type = edge.start_node.properties['type']
        end_type = edge.end_node.properties['type']
        node_type.append(start_type)
        node_type.append(end_type)
...
def getUseDefVarByPDG(db, pdg):
    ...
    for node in pdg.vs:
        if node['type'] == 'Function' or node['type'] == 'FunctionDef':
            continue
...
def getAllFuncfileByTestID(db, testID):
    ...
    # query_str = "queryNodeIndex('type:File').id"
    results = db.runGremlinQuery(query_str)
    return results
...
def get_all_calls_node(db, testID):
    ...
    for func_id in list_all_funcID: # all file in a testID
...
def getCallGraph(db, testID):
    ...
    for func_node in list_all_func_node:
        ...
        if function_name.find('::') != -1:  # if is a function in class, have two problems
            ...
            elif func_name.replace('~', '') == classname:
                list_callee_id = []
                list_delete_node = get_all_delete_node(db, testID)
                if list_delete_node == False:
                    continue
        ...
        if list_callee_id == []:
            ...
            if tag == 'func':
                ...
                for node in list_callee_CFGNode:
                    ...
                    var = str(node._id)
                    call_g = addDataEdge(call_g, startNode, endNode, var)
            else:
