...
def program_slice_backwards(pdg, list_startNode):  # startNode is a list
    ...
    for startNode in list_startNode:
        list_node = [startNode]
        not_scan_list.append(startNode['name'])
        predecessors = startNode.predecessors()  # 寻找前驱节点(父节点)
        if predecessors != []:
            for p_node in predecessors:
                list_node, not_scan_list = sub_slice_backwards(p_node, list_node, not_scan_list)
...
def program_slice_forward(pdg, list_startNode):#startNode is a list of parameters, only consider data dependency
    # pdg = del_ctrl_edge(pdg)  # 删除控制依赖边，在提取前向切片时只考虑数据依赖关系
            ...
            if ret:
                for funcname in ret:
                    if funcname.find('->') != -1:
                        ...
                        pdg = getFuncPDGByNameAndtestID(funcname, testID)  # 根据函数id查找对应的pdg文件
        ...
        if iter_time == 3 or iter_time == -1:  # allow cross 3 funcs:
            ...
            if list_node[1]['type'] == 'Parameter':   
                ...
                if func_name not in _dict.keys():
                    ...
                    iter_time += 1
                    _new_list = []
                    for my_index, item in enumerate(dict_func_pdg.items()):
                        targetpdg = item[1]
                        startnode = []
                        ...
                        # edges[list_cfgNodeID[my_index][1]] = backward_edge
                ...
                if funcname.find("::") > -1:
                    ...
                    if func_name not in _dict.keys():
                        ...
                        for my_index, item in enumerate(dict_func_pdg.items()):
                            ...
                            if startnode == []:
                                continue
                            backward_edge, ret_list = program_slice_backwards(targetPDG, startnode)
                            edges[list_cfgNodeID[my_index][1]] = backward_edge
                            not_scan_func_list.append(startnode[0]['name'])
                        ...
                        if _new_list != []:
                            del list_tuple_results_back[i]
                            list_tuple_results_back = list_tuple_results_back + _new_list
                            edges, list_tuple_results_back, not_scan_func_list = process_crossfuncs_back_byfirstnode(edges, list_tuple_results_back, testID, i, not_scan_func_list)
