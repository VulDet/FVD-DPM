...
def getReturnVarOfAPI(code):   # 获取API函数参数信息
...
def getFuncCFGById(testID, cfg_funcid):
    file_dir = os.path.join("./cfg", testID)

    for _file in os.listdir(file_dir):
        func_id = _file.split('_')[-1]

        if func_id == cfg_funcid:
            cfg_path = os.path.join(file_dir, _file) + '/cfg'
            print(cfg_path)
            f = open(cfg_path, 'rb')
            try:
                cfg = pickle.load(f)
...
def getFuncPDGById(testID, pdg_funcid):
    file_dir = os.path.join("./pdg", testID)
